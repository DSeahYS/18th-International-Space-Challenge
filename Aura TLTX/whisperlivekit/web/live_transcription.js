...
const russianTranscriptDiv = document.getElementById("russianTranscript");
...
function renderLinesWithBuffer(
  ...
  if (current_status === "no_audio_detected") {
    englishTranscriptDiv.innerHTML =
      "<p style='text-align: center; color: var(--muted); margin-top: 20px;'><em>No audio detected...</em></p>";
    russianTranscriptDiv.innerHTML = "";
    return;
  }
  ...
  (lines || []).forEach((item, idx) => {
    ...
    // English part
    let englishLine = currentLineText.trim().length > 0 || speakerLabel.length > 0
      ? `<p>${speakerLabel}<br/><div class='textcontent'>${currentLineText}</div></p>`
      : `<p>${speakerLabel}<br/></p>`;
    englishHtml += englishLine;

    // Russian part
    if (item.translation) {
      russianHtml += `<p><div class='textcontent'>${item.translation}</div></p>`;
    } else {
      russianHtml += `<p><div class='textcontent'></div></p>`; // empty to align
    }
  });

  englishTranscriptDiv.innerHTML = englishHtml;
  russianTranscriptDiv.innerHTML = russianHtml;
